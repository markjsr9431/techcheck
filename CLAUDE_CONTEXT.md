...
Nuevo flujo de diagnóstico "Laptop no carga" — completado.

## Completado

- Arquitectura base, Home, Design System.
- Diagnósticos: PC lenta, No enciende, Sin Internet, Pantalla negra, Posible virus, Disco duro, Pantalla azul, Sin sonido, Laptop no carga, Otro.
...
Nuevo flujo de diagnóstico "Laptop no carga": misma arquitectura que los demás diagnósticos (componente de flow con `DiagnosticResult`/`DiagnosticResultView`), integrado en la pantalla de selección. Categoría "Laptop no carga" habilitada en Home (`available: true`), etiqueta "Próximamente" ya no se muestra para esta categoría. Build verificado sin errores.
